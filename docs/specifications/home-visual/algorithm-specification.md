...
Quiet Space  
→ Subtle Fluctuation  
→ LightField Formation  
→ Wave Propagation  
→ Wavicle Localization / Spread  
→ Flow Alignment  
→ Residual State  
...
## 4. Common Mathematical Conventions
...
Local BendはBase Flowを壊さない程度に抑える。
...
## 6.2 Rendering Balance

実際の描画では以下を初期目安とする。

Persistent chromatic area：
...
WaveはField Vector Flowを基礎として移動する。
...
Waveは主に **Luminous Band** として描画する。

必要に応じて、

- multiple low-opacity strokes
- phase-like variation
- density variation
- subtle trail
- local glow
...
Luminous Band内部の微かなSpectral Transitionとして使用する。
...
# 9. Residual State and Repeat Cycle
...
残ったField / Wavicleから
次CycleのSubtle Fluctuationへ接続する。
...
```text
Quiet / Subtle Fluctuation   0.0–2.5 sec
LightField Formation         2.0–8.0 sec
WaveField Propagation        7.0–15.0 sec
Wavicle Birth / Spread      11.0–19.0 sec
Flow Alignment              14.0–22.0 sec
Residual State              18.0–26.0 sec
Next-cycle Transition       24.0–28.0 sec
```

初回のみQuiet Spaceを明確に見せる。

二周目以降はResidual Stateから
次CycleのSubtle Fluctuationへ接続する。
...
**StarCanvas  
+ one visible Canvas 2D  
+ one low-resolution offscreen buffer  
+ HTML**
...
```text
StarCanvas
└─ existing Far Space canvas

HomeVisual
├─ visible Canvas 2D
│  ├─ LightField offscreen buffer composite
│  ├─ WaveField
│  └─ ParticleField / Wavicle
│
└─ HomeVisualContent
...
visible Canvas 2Dへ描画する。

複数のlow-opacity strokeやLuminous Bandを重ねて表現する。
...
# 15. Component / Scheduler Architecture
...
## 16.3 Performance Degradation Order
...
- Final GeometryはAdaptive Constellationとする
- First ImplementationはLightFieldから開始する

---

# 22. Pending / Visual Tuning
...
- stability time
- Adaptive Constellation詳細
- Geometry rendering
- detailed Dissolution
- R3F
- OrbitControls
