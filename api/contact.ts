...
function isRateLimited(ipAddress: string, now: number) {
  ...
  if (!current || now - current.windowStartedAt >= rateLimitWindowMs) {
    rateLimitStore.set(ipAddress, { count: 1, windowStartedAt: now });
    return false;
  }
...
function getBody(request: VercelRequest): ContactRequestBody | null {
  if (!request.body || Array.isArray(request.body)) return null;
  ...
  if (typeof request.body === 'object') return request.body as ContactRequestBody;
  return null;
}
...
function sendGenericError(response: VercelResponse, statusCode = 400) {
...
export default async function contactHandler(request: VercelRequest, response: VercelResponse) {
  ...
  if (!body || Buffer.byteLength(JSON.stringify(body), 'utf8') > maxBodyBytes) {
    return sendGenericError(response, 413);
  }
